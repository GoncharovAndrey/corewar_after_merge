/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   add.c                                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hwiegand <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/08 15:19:27 by hwiegand          #+#    #+#             */
/*   Updated: 2019/07/18 18:23:01 by hwiegand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

int				add(t_car *car, t_map *map)
{
	int			reg_one;
	int			reg_two;
	int			reg_three;

	car->distance_from_command += 2;
	reg_one = get_value_from_argument(map, car, 1, GET_BYTE);
	reg_two = get_value_from_argument(map, car, 2, GET_BYTE);
	reg_three = get_value_from_argument(map, car, 3, GET_BYTE);
	car->arr_reg[reg_three - 1] = car->arr_reg[reg_one - 1]
			+ car->arr_reg[reg_two - 1];
	car->carry = (!car->arr_reg[reg_three - 1]) ? 1 : 0;
	return (0);
}
